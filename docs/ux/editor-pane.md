...
| --- | --- | --- |
| **Pack** (`editorMode === "pack"`) | If a **local pack** is selected: `PackStructurePane` (DB-backed structure for that project pack). | If pack mode is on but no local pack is selected, a placeholder prompts the user to pick a pack from Local Packs. |
